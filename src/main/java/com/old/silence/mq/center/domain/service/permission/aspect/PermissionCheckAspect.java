...
public class PermissionCheckAspect {
    ...
    public void checkPermission(JoinPoint joinPoint, RequirePermission requirePermission) {
        ...
        try {
            ...
            // 获取用户ID
            Long userId = getUserId(joinPoint, requirePermission);
            if (userId == null) {
     ...
     * 2. 从 SecurityContext 中获取（Spring Security）
     * 3. 从请求头中获取（自定义 Header）
     */
    private Long getUserId(JoinPoint joinPoint, RequirePermission requirePermission) {
        String userIdParamName = requirePermission.userIdParamName();
        
        // 方式1：从方法参数中获取
        if (userIdParamName != null && !userIdParamName.isEmpty()) {
            Long userId = getParameterValue(joinPoint, userIdParamName, Long.class);
            if (userId != null) {
                return userId;
            }
        }

        // 方式2：从 SecurityContext 中获取（Spring Security）
        try {
            ...
            if (authentication != null && authentication.getPrincipal() != null) {
                ...
                // 如果是自定义的 UserDetails，可以尝试获取 userId
                if (principal instanceof UserInfo) {
                    return ((UserInfo) principal).getUserId();
                }
            }
        } catch (Exception e) {
            logger.debug("从 SecurityContext 获取用户ID失败", e);
        }
     ...
     * 从方法参数中获取
     */
    private Long getTopicId(JoinPoint joinPoint, String topicIdParamName) {
        return getParameterValue(joinPoint, topicIdParamName, Long.class);
    }
