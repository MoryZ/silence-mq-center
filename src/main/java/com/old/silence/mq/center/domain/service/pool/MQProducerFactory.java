...
public class MQProducerFactory implements PooledObjectFactory<DefaultMQProducer> {
    ...
    public boolean validateObject(PooledObject<DefaultMQProducer> p) {
        ...
        // 检查 Producer 状态
        try {
            // RocketMQ 5.1.0+: 检查服务状态是否为 RUNNING
            ServiceState state = producer.getDefaultMQProducerImpl().getServiceState();
            if (state != ServiceState.RUNNING) {
                logger.warn("Producer service state is not RUNNING: {}", state);
                return false;
            }
