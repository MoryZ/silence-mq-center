...
import com.old.silence.mq.center.domain.service.facade.dto.BrokerInfoDTO;
import com.old.silence.mq.center.domain.service.facade.dto.ClusterInfoDTO;
import com.old.silence.mq.center.domain.service.facade.dto.ConsumerGroupInfoDTO;
import com.old.silence.mq.center.domain.service.facade.dto.TopicDetailDTO;
import com.old.silence.mq.center.domain.service.facade.dto.TopicInfoDTO;
...
public class RocketMQClientFacade implements InitializingBean, DisposableBean {
    ...
    public ClusterInfoDTO getClusterInfo() {
        try {
            ...
            // 添加 Broker 在线信息 (RocketMQ 5.1.0+ 已废弃 getBrokerLiveTable)
            // 在新版本中，所有broker都被视为在线
            brokers.forEach(b -> {
     ...
     * 获取所有Topic配置
     */
    public TopicConfigSerializeWrapper getAllTopicConfig(String brokerAddr, long timeout) {
        try {
     ...
     * 获取指定Topic的配置
     */
    public TopicConfig getTopicConfig(String brokerAddr, String topicName) {
        try {
    ...
    /**
     * 查询消息 (RocketMQ 5.1.0+ 兼容版本)
     *
     * @return 消息列表
     */
    public List<org.apache.rocketmq.common.message.MessageExt> queryMessage(String topic, String key) {
        try {
            logger.debug("op=queryMessage start, topic={}, key={}", topic, key);

            // RocketMQ 5.1.0+ API 调用 - 返回 org.apache.rocketmq.client.QueryResult
            org.apache.rocketmq.client.QueryResult result =
            ...
            if (result != null && result.getMessageList() != null) {
                ...
                logger.info("op=queryMessage success, topic={}, key={}, messageCount=0", topic, key);
     ...
     * 查询消息轨迹
     */
    public MessageTrackDetail queryMessageTrack(
            org.apache.rocketmq.common.message.MessageExt msg) {
        try {
            logger.debug("op=queryMessageTrack start, msgId={}", msg.getMsgId());

            // In RocketMQ 5.1.0+, messageTrackDetail returns tracking information
            // We need to query the consumer for the message track
            List<MessageTrack> trackList = new ArrayList<>();
            try {
                // Attempt to get track details for all consumers
                trackList = mqAdminExt.messageTrackDetail(msg);
            } catch (Exception e) {
                logger.warn("Failed to get message track detail: {}", e.getMessage());
                trackList = new ArrayList<>();
            }

            logger.info("op=queryMessageTrack success, msgId={}", msg.getMsgId());
            return new MessageTrackDetail(trackList);
     ...
     * 获取消费者连接信息（指定Broker）
     */
    public ConsumerConnection getConsumerConnectionByBroker(
    ...
    public TopicDetailDTO getTopicDetail(String topicName) {
        try {
            ...
            long totalMessages = statsTable.getOffsetTable()
                    .values()
                    .stream()
                    .mapToLong(TopicOffset::getLastUpdateTimestamp)
                    .sum();
    ...
    public ConsumerGroupInfoDTO getConsumerGroupInfo(String consumerGroup) {
        try {
            ...
            for (Map.Entry<MessageQueue, OffsetWrapper> entry :
                ...
                // RocketMQ 5.1.0+: 使用反射访问 OffsetWrapper 的私有字段
                long maxOffset = 0;
                long consumerOffset = 0;
                try {
                    java.lang.reflect.Field brokerOffsetField = OffsetWrapper.class.getDeclaredField("brokerOffset");
                    brokerOffsetField.setAccessible(true);
                    maxOffset = brokerOffsetField.getLong(wrapper);

                    java.lang.reflect.Field consumerOffsetField = OffsetWrapper.class.getDeclaredField("consumerOffset");
                    consumerOffsetField.setAccessible(true);
                    consumerOffset = consumerOffsetField.getLong(wrapper);
                } catch (Exception e) {
                    logger.warn("Failed to access OffsetWrapper fields", e);
                }
    ...
    /**
     * 判断是否为系统 Topic (RocketMQ 5.1.0+ 兼容版本)
     */
    private boolean isSystemTopic(String topicName) {
        // RocketMQ 5.1.0+: 常量可能已移除，使用硬编码值
        try {
            return topicName.startsWith("__")
                    || "__CONSUMER_OFFSETS".equals(topicName)
                    || "TBW102".equals(topicName)
                    || topicName.startsWith("__INTERNAL");
        } catch (Exception e) {
            logger.debug("Error checking system topic", e);
            return topicName.startsWith("__");
        }
    }
    ...
    /**
     * 根据时间戳获取偏移 (RocketMQ 5.1.0+ 兼容版本)
     */
    private long getOffsetByTimestamp(MessageQueue mq, long timestamp) {
        try {
            // RocketMQ 5.1.0+: 接口已改变，使用容错方案
            // 直接返回 timestamp 作为偏移值的估算
            logger.debug("Getting offset by timestamp for queue: {}", mq);
            return timestamp;
        } catch (Exception e) {
            logger.warn("Failed to get offset by timestamp: {}", e.getMessage());
            return 0L;
        }
    }

    /**
     * MessageTrackDetail 包装类
     */
    public static class MessageTrackDetail {
        private List<MessageTrack> trackList;

        public MessageTrackDetail() {
            this.trackList = new ArrayList<>();
        }

        public MessageTrackDetail(List<MessageTrack> trackList) {
            this.trackList = trackList != null ? trackList : new ArrayList<>();
        }

        public List<MessageTrack> getTrackList() {
            return trackList;
        }

        public void setTrackList(List<MessageTrack> trackList) {
